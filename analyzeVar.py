...
def main(rtwFile, mapFile, path, file, filter, project):
    ...
    print("Running ifnames tool to find c/cpp files that support variability. Please wait...")
    pc.runIfNames()
    # save variability features and files
    pc.saveVarFeaturesAndFiles()
    print(f"Total time for finding cpp files and variability features: {time.time() - start} seconds")
    cur_time = time.time()
    # run PCLocator to identify presence conditions in variability code
    print("Running PCLocator tool to identify presence conditions. Please wait...")
    pc.runPCLocator()
    print(f"Total time for running PCLocator: {time.time() - cur_time} seconds")
    cur_time = time.time()
    print("Generating feature model. Please wait...")
    rtw = RTW(rtwFile, mapFile)
    #rtw.showRTWConstraints()
    #rtw.showSATFormula()
    print(f"Total time for feature model generation: {time.time() - cur_time} seconds")
    cur_time = time.time()
    print("Analyzing presence conditions. Please wait...")
    ...
    print(f"Total time for presence condition identification and analysis: {time.time() - cur_time} seconds")
    pc.cleanup()
    cur_time = time.time()
    print("Finding min configuration sets. Please wait...")
    ...
    print(f"Total time for min configuration sets finding: {time.time() - cur_time} seconds")
    print(f"Total time for variability analysis: {time.time() - start} seconds")
    print()
    exit()
    if project == "axtls":
