...
def main(rtwFile, mapFile, path, file, filter, project):
    ...
    print(f"Total time for finding cpp files and variability features: {(time.time() - start):.2f} seconds")
    cur_time = time.time()
    ...
    pc.runPCLocator()
    print(f"Total time for running PCLocator: {(time.time() - cur_time):.2f} seconds")
    cur_time = time.time()
